...
@njit(cache = True)
def main_loop(N,q,dict,R0,R1,R2,flag):
    visited_indices = set()
    ...
    for index in dict.copy():  
        if index not in visited_indices:
            ...
            if orbit[0] != 3:
                ...
                read_and_write(dict, current_R,orbit,flag)

            else: pass


@njit(cache = True)
def main_loop_cz(N,q1,q2,dict,R0,R1,R2,flag):
    visited_indices = set()
...
@njit
def read_and_write(dict, current_R, orbit,flag):
...
class Simulator3:
    ...
    def simulate(self, circuit, measurement_vector = None, rho_vector = None, verbose = False):
        ...
        while (xi + xj) <= (self.num_gates)-1:
            ...
            if len(measurement_vector) <= len(rho_vector):
                x = -(xj+1)
                self.apply_gate(measurement_vector, x, flag = 1)
                xj+=1 
                self.msm_lengths.append(len(measurement_vector))
            else: 
                x = xi
                self.apply_gate(rho_vector, x, flag = 0)
                xi +=1
                self.rho_lengths.append(len(rho_vector))
    ...
    def apply_gate(self,dict, x, flag):
            ...
            if gate_type == 'CZ':
                ...
                main_loop_cz(self.N, qubits[0],qubits[1], dict,R0,R1,R2, flag)
                ...
                main_loop(self.N, qubits[0], dict,R0,R1,R2, flag)
...
# N,n,m = 300,10,5
# circuit = mod_trotter_circuit(n,m,1,0.3,0.9,1)
# simulator = Simulator3(2*n)
