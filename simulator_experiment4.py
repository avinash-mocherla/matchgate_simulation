...
#         simulator.simulate(circuit,threshold = 0.01)
#         f.append(np.average(np.abs(list(simulator.measurement_vector.values()))))
        

#         simulator = Simulator(N, pruned = False)
#         simulator.simulate(circuit)
#         l.append(np.average(np.abs(list(simulator.measurement_vector.values()))))

#     depths.append(d[-1])        

#     results.append(np.average(l)-np.average(f))
...
#         d.append(len(circuit))
#         simulator = Simulator(N, pruned = True)
#         simulator.simulate(circuit,threshold = 0.001)
#         f.append(np.average(np.abs(list(simulator.measurement_vector.values()))))
...
#     # depths1.append(d[-1])        
#     results1.append(np.average(l)-np.average(f))


# plt.scatter(depths,results, label = '0.01')
# plt.scatter(depths,results1, label = '0.001')
# plt.title('absolute difference between average pruned vector element and average exact vector element')
# plt.xlabel('depth')
# plt.ylabel('absolute difference in average element, averaged over 50 runs')
# plt.legend()
# plt.show()
...
for i in range(1,g):
    ...
    for r in range(repeats):
        ...
        simulator = Simulator(N, pruned = True)
        simulator.simulate(circuit,threshold = 0.001)
        f.append(np.sum((list(simulator.measurement_vector.values()))))
        

        simulator = Simulator(N, pruned = False)
        simulator.simulate(circuit)
        l.append(np.sum((list(simulator.measurement_vector.values()))))

    depths.append(d[-1])        

    results.append(np.abs(np.average(l)-np.average(f)))
...
#         simulator = Simulator(N, pruned = True)
#         simulator.simulate(circuit,threshold = 0.001)
#         f.append(np.sum(np.abs(list(simulator.measurement_vector.values()))))
        

#         simulator = Simulator(N, pruned = False)
#         simulator.simulate(circuit)
#         l.append(np.sum(np.abs(list(simulator.measurement_vector.values()))))
...
plt.scatter(depths,results, label = '0.01')
# plt.scatter(depths,results1, label = '0.001')
plt.title('absolute difference between sum of pruned vector element and sum of exact vector element')
plt.xlabel('depth')
plt.ylabel('absolute difference in sum of elements')
plt.legend()
plt.show()
