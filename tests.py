...
#Test 1: Comparison of pruned and normal pauli rank:

def test1(g, N, s, repeats, tols = [0,0.001,0.01,0.1]): 

    paulis_pruned1, paulis_pruned2, paulis, depths = [], [], [],[]
    ...
    for tol in tols:
        ...
        for i in range(1,g):
            ...
            for r in range(repeats):
                ...
                circuit = layered_circuit(i,N,s)
                d.append(len(circuit))

                simulator = Simulator4(N)
                simulator.simulate(circuit,threshold = tol, verbose = False)
                temp1.append(simulator.msm_lengths[-1])
            
            paulis.append(np.average(temp1))
            depths.append(d[-1])

        plt.scatter(depths,paulis, label = str(tol))
    plt.title('Pauli rank versus depth')
    plt.xlabel('depth')
    plt.ylabel('Final pauli rank')
    plt.legend()
    plt.show()

# Distribution of coefficients for random circuit, layered and trotter-like circuit 
def test2():
    
    N_qubits = 8
    circuit2 = layered_circuit(50, N_qubits, 2)
    circuit1 = random_type_circuit(N_gates = len(circuit2), N_qubits = N_qubits, N_swaps= 2)
    circuit3 = trotter_circuit(int(N_qubits/2), N_steps = 3, J = 0.3, U = 0.9, tau =1)


    # print(len(circuit1),len(circuit2),len(circuit3))

    simulator = Simulator(N_qubits, pruned = False)
    simulator.simulate(circuit1, verbose =True)
    l1 = simulator.measurements

    simulator1 = Simulator(N_qubits, pruned = False)
    simulator1.simulate(circuit2,verbose = True)
    l2 = simulator1.measurements

    
    simulator2 = Simulator(N_qubits, pruned = False)
    simulator2.simulate(circuit3,verbose =True)
    l3 = simulator2.measurements

    fig, axes = plt.subplots(3,3, figsize = (12,12))

    axes[0][0].hist(np.abs(l1[1]),bins = 1000, density = True)
    
    axes[0][1].hist(np.abs(l1[int(len(circuit1)/2)]),bins = 1000,density  = True)
    axes[0][2].hist(np.abs(l1[-1]),bins = 1000, density = True)

    axes[1][0].hist(np.abs(l2[1]),bins = 1000, density = True)
    axes[1][1].hist(np.abs(l2[int(len(circuit2)/2)]),bins = 1000, density = True)
    axes[1][2].hist(np.abs(l2[-1]),bins = 1000, density= True)

    axes[2][0].hist(np.abs(l3[1]),bins = 1000, density = True)
    axes[2][1].hist(np.abs(l3[int(len(circuit3)/2)]),bins = 1000, density = True)
    axes[2][2].hist(np.abs(l3[-1]),bins = 1000, density= True)

    axes[0][0].set_title('Random circuit N = %s , depth = %s'%(N_qubits,1))
    axes[0][1].set_title('Random circuit N = %s ,   depth = %s'%((N_qubits, int(len(circuit1)/2))))
    axes[0][2].set_title('Random circuit N = %s ,  depth = %s '%(N_qubits,len(circuit1)))
    axes[1][0].set_title('Layered circuit N = %s , depth = %s  '%(N_qubits,1))
    axes[1][1].set_title('Layered circuit N = %s ,  depth = %s '%(N_qubits,int(len(circuit2)/2)))
    axes[1][2].set_title('Layered circuit N = %s ,  depth = %s '%(N_qubits,len(circuit2)))
    axes[2][0].set_title('Trotter circuit N = %s ,  depth = %s '%(N_qubits,1))
    axes[2][1].set_title('Trotter circuit N = %s ,  depth = %s '%(N_qubits,int(len(circuit3)/2)))
    axes[2][2].set_title('Trotter circuit N = %s ,  depth = %s '%(N_qubits,len(circuit3)))

    


    fig.suptitle(r'Distribution of coefficients at depth = 0,$\frac{N}{2}$,N')
    fig.supxlabel('Value of coefficient')
    fig.supylabel('Probability denisty')

    fig.tight_layout(pad=4.0)
    plt.show()

#Distribution of coefficients for pruned trotter, random and layered circuits
...
def test3():
    ...
    circuit2 = layered_circuit(50, N_qubits, 2)
    circuit1 = random_type_circuit(N_gates = len(circuit2), N_qubits = N_qubits, N_swaps= 2)
    circuit3 = trotter_circuit(int(N_qubits/2), N_steps = 10, J = 0.3, U = 0.9, tau =1)


    # print(len(circuit1),len(circuit2),len(circuit3))

    simulator = Simulator(N_qubits, pruned = True)
    simulator.simulate(circuit1, verbose =True, threshold =0.01)
    l1 = simulator.measurements

    simulator1 = Simulator(N_qubits, pruned = True)
    simulator1.simulate(circuit2,verbose = True, threshold =0.01)
    l2 = simulator1.measurements

    
    simulator2 = Simulator(N_qubits, pruned = True)
    simulator2.simulate(circuit3,verbose =True, threshold=0.01)
    l3 = simulator2.measurements
...
#Accuracy versus pauli rank graph for different depths
def test4():
    J = 0.3
    U = 0.9
    tau = float(0.3/J)
    ...
    params = [(0,0,6,2), (0,1,6,4), (1,0,6,6), (1,1,6,8)]
...
# test4()
#Observation of separated dynamics of spin and charge in the Fermi-Hubbard Model: 
...
def test5():
    ...
    def analyse(measurements):
            ...
            # n_down = 0.5 * (1 - np.array(measurements[0:N_sites]) )
            # n_up = 0.5 * (1 - np.array(measurements[N_sites:]))
    ...
    for k,ax_inds in enumerate([(0,0,0),(0,1,4),(1,0,6),(1,1,10)]):
    # for k,ax_inds in enumerate([(0,0,0)]):
        charge_densities = []
        spin_densities = []

        for j in range(repeats):

            circuit = trotter_circuit(N_sites, ax_inds[2], J = J,U =U,tau = tau)
            ...
            for i in range(2*N_sites): 
                ...
                simulator = Simulator(N = 2*N_sites, pruned = True)
                z_measurement = simulator.simulate(circuit, measurement_vector=measurement_vector, threshold = 0.01)
                # z_measurement1 = simulator.simulate(circuit, measurement_vector=measurement_vector, threshold = 0)
                # print(z_measurement,z_measurement1)
                l.append(z_measurement)
        
            charge_density, spin_density = analyse(l)
            charge_densities.append(charge_density)
            spin_densities.append(spin_density)
        
        charge_densities_avg = np.average(charge_densities,axis=0)
        spin_densities_avg = np.average(spin_densities,axis=0)
        charge_densities_std  = np.std(charge_densities) / np.sqrt(np.size(charge_densities))
        spin_densities_std  = np.std(spin_densities) / np.sqrt(np.size(spin_densities))


        twin_ax = twin_axes[k]
        # axs[ax_inds[0]][ax_inds[1]].scatter(range(N_sites),charge_densities, c= 'blue')
        # twin_ax.scatter(range(N_sites),spin_densities, c = 'orange')
        axs[ax_inds[0]][ax_inds[1]].errorbar(range(N_sites),charge_densities_avg, yerr = charge_densities_std, color = 'blue',  ecolor = 'blue', linestyle='',fmt='o',alpha=0.3)
        twin_ax.errorbar(range(N_sites), spin_densities_avg, yerr = spin_densities_std, color = 'red', ecolor = 'red', linestyle='',fmt='o',alpha=0.3)
    plt.show()
...
def test6():
    ...
    for n in [3,4]:
        ...
        for d in depths:
            # print(n,d)
            circuit = trotter_circuit(n,d,0.3,0.9,1)
            simulator = Simulator(N = 2*n, pruned = True)
            start_time = time.time()
            res = simulator.simulate(circuit, threshold = 0.0)
            time_taken = time.time() - start_time
            simulator = Simulator(N = 2*n, pruned = True)
            res1 = simulator.simulate(circuit, threshold = 0.0, verbose = True)
            error = np.abs(res1 - res)
            times.append(time_taken)
            errors.append(error)
            
        data.append(times)
        axs[0].scatter(depths, times, label = '%s'%n, marker = 'x', s = 8)
        axs[1].scatter(depths, errors, label = '%s'%n, marker = 'x', s = 8)
    

    axs[0].set_xlabel('Trotter steps')
    axs[0].set_ylabel('Time taken')
    axs[0].set_title(r'Trotter circuit simulation with pruning $\epsilon = 0.001$')
    axs[0].grid(True)
    axs[0].set_yscale('log')
    axs[0].legend(loc = 0, ncol = 2, fontsize = 'small', title = 'Num. sites')
    
    axs[1].set_xlabel('Trotter steps')
    axs[1].set_ylabel('Error')
    axs[1].grid(True)
    axs[1].set_yscale('log')
    axs[1].legend(loc = 0, ncol = 2, fontsize = 'small', title = 'Num. sites')
    fig.tight_layout(pad=1)
    plt.show()


def test7():
    # Max size, Error?, Time taken.
    ...
    for n in [2,4,8,10]:
        ...
        for d in depths:
            ...
            simulator = Simulator(N = 2*n, pruned = True)
            start_time = time.time()
            res = simulator.simulate(circuit, threshold = 0.1, verbose= True)
            time_taken = time.time() - start_time
        
            total_pauli_length = sum(simulator.lengths)
  

            times.append(time_taken)
            lengths.append(total_pauli_length)
        
            

        # print(lengths)
        axs.scatter(depths, lengths, label = '%s'%n, marker = 'x', s = 8)
        # axs.scatter(depths, times, label = '%s'%n, marker = 'x', s = 8)
        axs.plot(depths, lengths, linestyle='dashed')

    axs.set_xlabel(r'Trotter steps', fontsize=14)
    axs.set_ylabel(r'Time taken (s)',fontsize=14)
    axs.set_title(r'Trotter circuit simulation with pruning $\epsilon = 0.001$', fontsize = 14)
    axs.grid(True)
    axs.set_yscale('log')
    axs.legend(loc = 0, ncol = 2, fontsize = 'small', title = 'Num. sites')

    fig.tight_layout(pad=1)
    # plt.savefig("trotter_pruned.png", transparent=True)
    plt.show()
...
#     top_ax.set_ylabel(r'$log(\chi_{max})$')
#     right_ax.set_xlabel(r'$log(\chi_{max})$')
...
#     x = np.arange(2,10,1) #--------------------------------------------------Problem with doubling here-----------------------------------------------------------------------------------
#     y = np.arange(len(x))

#     z = np.empty(shape = (len(x), len(y)))
#     for i,n in enumerate(x):
#         for j,m in enumerate(y):
#             z[i][j] = np.log10(len_L(n,m))
...
#     im = main_ax.imshow(z, origin='lower')
...
#     v_line = main_ax.axvline(cur_x1, color='red')
#     h_line = main_ax.axhline(cur_y1, color='blue')
#     v_prof, = right_ax.plot(z[:,int(cur_x1)],np.arange(y.shape[0]), 'red')
#     h_prof, = top_ax.plot(np.arange(x.shape[0]),z[int(cur_y1),:], 'blue')

#     v_line = main_ax.axvline(cur_x2, color='darkred')
#     h_line = main_ax.axhline(cur_y2, color='green')
#     v_prof, = right_ax.plot(z[:,int(cur_x2)],np.arange(y.shape[0]), 'darkred')
#     h_prof, = top_ax.plot(np.arange(x.shape[0]),z[int(cur_y2),:], 'g-')

#     right_ax.grid('both')
#     top_ax.grid('both')

#     main_ax.set_xticks(range(len(x)))
#     main_ax.set_yticks(range(len(y)))
#     main_ax.set_xticklabels(x)
#     main_ax.set_yticklabels(y)


#     plt.savefig("contour_exact.png", transparent=True)
#     plt.show()
...
#     divider = make_axes_locatable(main_ax)
#     top_ax = divider.append_axes("top", 1.1, pad=0.1,sharex=main_ax)
#     right_ax = divider.append_axes("right", 1.1,pad=0.1,sharey=main_ax)

#     # make some labels invisible
#     top_ax.xaxis.set_tick_params(labelbottom=False)
#     right_ax.yaxis.set_tick_params(labelleft=False)

#     main_ax.set_xlabel('Number of Sites')
#     main_ax.set_ylabel('Trotter Steps')
#     top_ax.set_ylabel(r'$log(\chi_{avg})$')
#     right_ax.set_xlabel(r'$log(\chi_{avg}) $')
...
#             temp = []
#             for r in range(repeats):
#                 circuit = mod_trotter_circuit(N_sites = n, N_steps = m, N_swaps =1, J = 0.3, U = 0.9, tau = 1)
#                 simulator = Simulator(n, pruned = True)
#                 simulator.simulate(circuit, threshold = 0, verbose = True)
#                 average_pauli_rank = np.average(simulator.lengths)
#                 temp.append(average_pauli_rank)
            
#             z[i][j] = np.log10(np.average(temp))

#     z1 = np.empty(shape = (len(x), len(y)))
#     for i,n in enumerate(x):
#         for j,m in enumerate(y):
#             z1[i][j] = np.log10(len_L(n,m))
...
#     v_line = main_ax.axvline(cur_x1, color='darkred')
#     h_line = main_ax.axhline(cur_y1, color='g')
    
#     v_line = main_ax.axvline(cur_x2, color='red')
#     h_line = main_ax.axhline(cur_y2, color='blue')

#     right_ax.scatter(z[:,int(cur_x1)],np.arange(y.shape[0]), c = 'darkred')
#     top_ax.scatter(np.arange(x.shape[0]),z[int(cur_y1),:], c = 'g')
#     right_ax.plot(z1[:,int(cur_x1)],np.arange(y.shape[0]), 'darkred')
#     top_ax.plot(np.arange(x.shape[0]),z1[int(cur_y1),:], 'g')

#     right_ax.scatter(z[:,int(cur_x2)],np.arange(y.shape[0]), c = 'red')
#     top_ax.scatter(np.arange(x.shape[0]),z[int(cur_y2),:], c = 'blue')
#     right_ax.plot(z1[:,int(cur_x2)],np.arange(y.shape[0]), 'red')
#     top_ax.plot(np.arange(x.shape[0]),z1[int(cur_y2),:], 'blue')
...
#     plt.savefig("contour1.png", transparent=True)
#     plt.show()


# def test10():
#     circuit = random_type_circuit(10,2, 0 ,0,0)
#     simulator = Simulator(3)
#     res = simulator.simulate(circuit)
#     res_hard = simulator.simulate_hard(circuit)
#     # print(res, res_hard)
